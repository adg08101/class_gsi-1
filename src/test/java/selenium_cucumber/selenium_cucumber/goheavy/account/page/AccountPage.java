...
public class AccountPage extends PageObject {
	...
	public WebElement get_required_field_message(String field, String inputType) {
		waitForSpinningElementDissapear();
		By alert = By.xpath("//" + inputType + "[@id='" + field + "']/ancestor::div["
				+ "contains(@class, 'ant-form-item-has-error')]/descendant::div[@role='alert']");
	...
	public boolean set_invalid_data_to_field(String field, String fieldType) {
		...
		try {
			waitForSpinningElementDissapear();
			By element_xpath = By.xpath("//" + fieldType + "[@id='" + field + "']");
			this.scroll(formScroll, element_xpath);
			if (field.contains("Name"))
				setWebElementText(element_xpath, getFaker().phoneNumber().cellPhone());
			else if (field.contains("Phone"))
				setWebElementText(element_xpath, "000 000");
			else if (field.contains("email"))
				setWebElementText(element_xpath, getFaker().name().fullName());
			else if (field.contains("Zip"))
